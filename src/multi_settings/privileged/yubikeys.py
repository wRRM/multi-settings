...
def validate_credential(username: str, credential: str) -> str:
    if len(credential) > 16_384 or "\n" in credential or "\r" in credential:
        raise ValidationError("The generated credential has an invalid format.")
    if not credential.startswith(f"{username}:"):
        raise ValidationError("The generated credential belongs to a different account.")
    fields = credential.split(":", maxsplit=1)[1].split(",")
    if len(fields) < 3 or any(not field.strip() for field in fields[:3]):
        raise ValidationError("The generated credential is incomplete.")
    return credential
...
def enroll_yubikey(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise ValidationError("Choose the primary or secondary slot.") from error
    ...
    if any(item.get("serial") == serial for item in enrollments):
        raise ValidationError(f"YubiKey {serial} is already enrolled.")
    if any(item.get("username") == username and item.get("slot") == slot.value for item in enrollments):
        raise ValidationError(f"{username} already has a {slot.value} YubiKey.")
    enrollments.append(
        {"username": username, "serial": serial, "slot": slot.value, "credential": credential}
    )
    rebuild_mapping_file(state)
    save_state(state)
    emit("complete", message=f"Enrolled YubiKey {serial} as {username}'s {slot.value} key.")
...
def unenroll_yubikey(payload: dict[str, Any]) -> None:
    ...
    if len(existing) == len(remaining):
        raise ValidationError(f"{username} has no {slot.value} YubiKey enrollment.")
    ...
    if account_needs_key and not still_enrolled:
        raise ValidationError(
            "Disable the affected password + YubiKey requirement before removing this account's last key."
        )
    state["enrollments"] = remaining
    rebuild_mapping_file(state)
    save_state(state)
    emit("complete", message=f"Removed {username}'s {slot.value} YubiKey enrollment.")
